...
public class StageController : MonoBehaviour
{
    ...
    public  Vector2Int          playerPosition;     // 플레이어 초기 위치 
    ...
    private void Awake()
    {
        ...
        // PlayerPrefs을 이용해 디바이스에 저장되어 있는 현재 스테이지 인덱스 불러오기
        ...
        // 현재 저장되어 있는 json 파일이 Stage01, Stage02.. json 파일 데이터를 불러온다
    ...
    public void GameClear()
    {
        ...
        {
            // index를 1 증가시키, 수정된 index 값을 디바이스에 "StageIndex" 키로 저장
            index++;
            PlayerPrefs.SetInt("StageIndex", index);

            // 현재 씬을 다시 로드
            // 씬이 다시 로드될 때 StageIndex 값이 바뀌었기 때문에 다음 스테이지의 .json 파일을 열게 된다
