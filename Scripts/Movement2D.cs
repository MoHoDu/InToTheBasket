...
public class Movement2D : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // 점프 업데이트 (바작을 밟으면 바로 점프가 되도록 JumpTo()를 계속 호출)
        // 추후 타일에서 호출하게 됨
        //JumpTo();
    }
    ...
    {
        // 왼쪽 or 오른쪽 이동 상태일 때 좌/우 방향키를 누르면
    ...
    /// <summary>
    /// y축 방향으로 광선 발사 (위/아래 이동)
    /// </summary>
    private void RaycastVertical(ref Vector3 velocity)
    {
        float        direction      = Mathf.Sign(velocity.y);               // 이동 방향 (오:1, 왼:-1)
    ...
    /// <summary>
    /// 한 쪽으로 발사되는 광선 사이의 간격 (광선 개수에 따라 달라짐)
    /// </summary>
